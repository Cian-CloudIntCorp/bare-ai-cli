...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (BARE_AI.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory reload loads BARE_AI.md files. When
...
```json
{
  ...
  "context": {
    "fileName": ["CONTEXT.md", "BARE_AI.md"],
...
(defaulting to `BARE_AI.md` but configurable via the `context.fileName` setting)
...
### Example context file content (e.g., `BARE_AI.md`)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (e.g., `BARE_AI.md`) from
  ...
  1.  **Global context file:**
      - Location: `~/.gemini/<configured-context-filename>` (e.g.,
        `~/.gemini/BARE_AI.md` in your user home directory).
