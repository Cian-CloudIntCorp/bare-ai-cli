...
learn how to define project-wide rules with `BARE_AI.md`, teach the agent
...
## How to define project-wide rules (BARE_AI.md)

The most powerful way to control the agent's behavior is through `BARE_AI.md`
...
1.  In the root of your project, create a file named `BARE_AI.md`.
...
1.  **Global:** `~/.gemini/BARE_AI.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./BARE_AI.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/BARE_AI.md` (Rules specific to the `src` folder).
...
`BARE_AI.md` files and saved memories), use the `/memory show` command.
...
If you edit a `BARE_AI.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Don't dump your entire internal wiki into `BARE_AI.md`.
...
- **Review often:** Periodically check your `BARE_AI.md` files to remove outdated
  rules.
