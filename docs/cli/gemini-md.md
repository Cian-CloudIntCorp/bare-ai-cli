# Provide context with BARE_AI.md files

Context files, which use the default name `BARE_AI.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.gemini/BARE_AI.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `BARE_AI.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `BARE_AI.md` files in that directory and its
...
### Example `BARE_AI.md` file

Here is an example of what you can include in a `BARE_AI.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `BARE_AI.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.gemini/BARE_AI.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `BARE_AI.md` files into smaller, more manageable
...
**Example `BARE_AI.md` with imports:**

```markdown
# Main BARE_AI.md file
...
While `BARE_AI.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "BARE_AI.md"]
  }
}
```
