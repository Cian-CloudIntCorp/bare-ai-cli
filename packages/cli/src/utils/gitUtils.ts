...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-bare-ai-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-bare-ai-cli release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest run-bare-ai-cli release on GitHub.`,
    );
  }
};
