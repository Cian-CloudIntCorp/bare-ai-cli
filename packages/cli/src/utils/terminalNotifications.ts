...
import { debugLogger, writeToStdout } from '@bare-ai/core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'Bare AI CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open Bare AI CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'Bare AI CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open Bare AI CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'Bare AI CLI session complete',
