...
import { debugLogger, isGitRepository } from '@bare-ai/core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        const brewPrefix = childProcess
          .execSync('brew --prefix bare-ai-cli', {
            encoding: 'utf8',
        ...
        // Brew is not installed or bare-ai-cli is not installed via brew.
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g @google/bare-ai-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @google/bare-ai-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/install/global')) {
      const updateCommand = 'bun add -g @google/bare-ai-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @google/bare-ai-cli@latest';
    return {
