...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('gemini')
    .usage(
      'Usage: gemini [options] [command]\n\nBare AI CLI - Defaults to interactive mode. Use -p/--prompt for non-interactive (headless) mode.',
    )
    .option('debug', {
      alias: 'd',
      type: 'boolean',
    ...
    .command('$0 [query..]', 'Launch Bare AI CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('resume', {
          ...
          coerce: (value: string): string => {
            // When --resume passed with a value (`bare-ai --resume 123`): value = "123" (string)
            // When --resume passed without a value (`bare-ai --resume`): value = "" (string)
