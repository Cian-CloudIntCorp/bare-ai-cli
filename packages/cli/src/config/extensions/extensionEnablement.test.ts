...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(ExtensionStorage.getUserExtensionsDir).mockReturnValue(
      path.join(testDir.path, BARE_AI_DIR),
    );
  ...
  it('should correctly prioritize more specific enable rules', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/bare-ai-cli');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/bare-ai-cli')).toBe(
      true,
    );
  });
