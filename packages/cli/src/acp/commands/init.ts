...
import { performInit } from '@bare-ai/core';
import type {
  Command,
  CommandContext,
  ...
  async execute(
    ...
    switch (result.type) {
      ...
      case 'submit_prompt':
        ...
        // or just seed the BARE_AI.md file as we've done above.
        return {
          name: this.name,
          data: {
            type: 'message',
            messageType: 'info',
            content: `A template BARE_AI.md has been created at ${geminiMdPath}.\n\nTo populate it with project context, you can run the following prompt in a new chat:\n\n${result.content}`,
          },
        };
